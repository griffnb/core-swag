...
func TestParseParamStructCodeExample(t *testing.T) {
	...
	import structs "github.com/griffnb/core-swag/testing/testdata/param_structs"
	`, goparser.ParseComments)
	assert.NoError(t, err)

	parser := New()
	// Use packages.ParseFile to load just the struct definitions without full API parsing
	err = parser.packages.ParseFile("github.com/griffnb/core-swag/testing/testdata/param_structs", "testdata/param_structs/structs.go", nil, ParseModels)
	assert.NoError(t, err)
